...
import { developer1, developer2, developerUpdate1 } from "../../mocks/developers.mock";
...
describe("PATCH - /developers/:id", () => {
  ...
  test("Sucesso - Atualizando um developer com sucesso.", async () => {
    ...
    const response = await supertest(app).patch(`/developers/${userId}`).send(developerUpdate1);
  ...
  test("Falha - Tentando atualizar um developer que não existe.", async () => {
    const response = await supertest(app).patch(`/developers/${999}`).send(developerUpdate1);
