...
import { developerInfo1, developerInfoNotAccepted } from "../../mocks/developerInfo.mock";
...
describe("POST - /developers/:id/infos", () => {
  ...
  it("Sucesso - Criando uma developerInfo com sucesso.", async () => {
    ...
    const response = await supertest(app).post(`/developers/${userId}/infos`).send(developerInfo1);
  ...
  it("Falha - Tentando cadastrar uma informação à um usuário que já possui informação atrelada à ele.", async () => {
    ...
    await supertest(app).post(`/developers/${userId}/infos`).send(developerInfo1);
  ...
  it("Falha - Tentando cadastrar preferedOS a um usuário que não existe", async () => {
    const response = await supertest(app).post(`/developers/999/infos`).send(developerInfoNotAccepted);
